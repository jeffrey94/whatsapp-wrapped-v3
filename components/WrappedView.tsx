...
import { ChevronUp, ChevronDown, RefreshCw, Share2, Download, Package, Presentation, X, Check, Menu, FileDown, Link, Copy, CheckCircle } from 'lucide-react';
...
export const WrappedView: React.FC<WrappedViewProps> = ({ data, onReset }) => {
    ...
    const slides = useMemo(() => {
        ...
        // Other slides
        slideList.push({ component: <OverviewSlide key="overview" data={data} />, title: "Vibe Check" });
        ...
        if (moments.length > 0) {
            moments.forEach((moment, i) => {
                slideList.push({
                    component: <MomentSlide key={`moment-${i}`} data={data} items={[moment]} momentIndex={i} totalMoments={moments.length} />,
                    title: `Moment ${i + 1}`
                });
            });
        } else {
            slideList.push({ component: <MomentSlide key="moment-empty" data={data} />, title: "Moments" });
        }
        ...
        slideList.push({ component: <PredictionSlide key="predictions" data={data} />, title: "Predictions" });
    ...
    const handleShareLink = async () => {
        ...
        try {
            ...
            alert('Failed to generate share link. Make sure the app is deployed on Vercel with KV configured.');
        } finally {
                ...
                <div className="fixed top-0 left-0 w-screen h-0 overflow-hidden" style={{ zIndex: -1 }}>
                    <div ref={exportContainerRef} style={{ width: '540px' }}>
                        {slides.map((slide, idx) => (
                            <div key={idx} style={{ width: '540px', height: '960px' }} className="relative overflow-hidden">
                                {slide.component}
                            </div>
                        ))}
                    </div>
            ...
            <div className="absolute top-0 left-0 right-0 z-50 safe-top px-3 pt-3">
                <div className="flex gap-[3px] w-full">
                    {slides.map((_, idx) => (
                        ...
                        >
                            <div
                                className={`h-full rounded-full transition-all duration-300 ${idx < currentSlide ? 'bg-white' :
            ...
            {showMenu && (
                <div className="absolute inset-0 z-[60] bg-black/90 backdrop-blur-xl flex flex-col items-center justify-center animate-fade-in">
                        ...
                        {!shareLink ? (
                            ...
                            <div className="w-full glass rounded-2xl p-4 space-y-3">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    >
                                        {linkCopied ? <CheckCircle size={18} className="text-green-400" /> : <Copy size={18} />}
                                    </button>
                                </div>
                    ...
                    <div className="mt-8 w-full max-w-xs px-6">
                        <div className="text-xs text-white/40 uppercase tracking-widest mb-3">Jump to Slide</div>
                        <div className="grid grid-cols-4 gap-2 max-h-48 overflow-y-auto no-scrollbar">
                            {slides.map((slide, idx) => (
                                <button
                                    ...
                                    className={`p-2 rounded-lg text-xs text-center transition-colors ${idx === currentSlide
